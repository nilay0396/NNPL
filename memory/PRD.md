...
- [x] **Feb 2026** — Auto-acknowledgement emails added. Every `/api/enquiries` and `/api/vendors` POST now dispatches TWO emails via Emergent-managed Resend: (a) the existing company notification to nilaynarayanpolychem@gmail.com and (b) a polite branded acknowledgement to the enquirer/applicant with expected response time, sales/ops/email/website contact, and — for gated TDS/SDS requests — an inline note confirming the document has been shared. Verified live: 6 emails dispatched across 3 test submissions (`Email sent to <enquirer> — id=…` + `Email sent to nilaynarayanpolychem@gmail.com — id=…` per submission). Backend pytest expanded to **63 / 63 passing** with 2 new log-based ack-email tests.
